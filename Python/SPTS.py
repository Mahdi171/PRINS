from charm.toolbox.pairinggroup import PairingGroup,ZR,G1,G2,GT,pair
from charm.toolbox.secretutil import SecretUtil
from PoK import PoK
from openpyxl import Workbook
from secretshare import SecretShare
from charm.core.engine.util import serializeDict

class SPTS():
    def __init__(self, groupObj):
        global util, group
        util = SecretUtil(groupObj)        
        group = groupObj
                        
    def PGen(self):
        g, h = group.random(G1), group.random(G2)   
        e_gh = pair(g,h)
        mpk = {'g':g, 'h':h, 'e_gh':e_gh}
        return (mpk)

    def kgen(self, mpk, k,n):
        sgk={}; vk={}; X={}; Y={}; alphaShare={}; betaShare={}
        SSS = SecretShare(group, True)
        alpha, beta = group.random(), group.random()
        alphashares = SSS.genShares(alpha, k, n)
        betashares = SSS.genShares(beta, k, n)
        for i in range(1,n+1):
            alphaShare[i] = alphashares[i]
            betaShare[i] = betashares[i]
            X[i] = mpk['h'] ** alphaShare[i]
            Y[i] = mpk['h'] ** betaShare[i]
        sgk = {'alpha': alphashares, 'beta': betashares}
        vk = {'X': X, 'Y':Y}
        pk = {'X': mpk['h']**alpha, 'Y': mpk['h']**beta}
        return (sgk, vk, pk)
    
    def iDH(self,mpk,id,m):
        gg=group.hash(id, G1)
        return {'id':id,'M1':gg**m,'M2':mpk['h']**m}

    def par_sign(self,mpk,sk,M,k):
        s={}
        gg=group.hash(M['id'], G1)
        if pair(gg,M['M2'])==pair(M['M1'],mpk['h']):
            for i in range(1,k+1): 
                s[i] = (gg**sk['alpha'][i])*(M['M1']**sk['beta'][i])
        return {'gg':gg, 's':s}

    def Par_verify(self,mpk,vk,pk,sigma,M,k):
        for i in range(1,k+1):
            if pair(sigma['gg'],M['M2']) == pair(M['M1'],mpk['h']) and \
            pair(sigma['s'][i],mpk['h']) == pair(sigma['gg'],vk['X'][i])*pair(M['M1'],vk['Y'][i]):
                return print("True")
        else:
            return print("Wrong")


    def reconst(self, sigma, k):
        SSS = SecretShare(group, True)
        s=group.init(G1, 1); list={}
        for i in range(1,k+1):
            list[group.init(ZR, i)]=group.init(ZR, i)
        keys= list.keys()
        coeff = SSS.recoverCoefficients(keys)
        for i in range(1,k+1):
            s *= sigma['s'][i] ** coeff[group.init(ZR, i)]
        return {'gg':sigma['gg'], 's':s}

    def verify(self,mpk,pk,M,sigma):
        if pair(sigma['gg'],M['M2']) == pair(M['M1'],mpk['h']) and \
            pair(sigma['s'],mpk['h']) == pair(sigma['gg'],pk['X'])*pair(M['M1'],pk['Y']):
                return 1
        else:
            return 0
    
    def Randomize(self, mpk, M, sigma):
        r = group.random()
        randomized_signature = {'gg':sigma['gg']**r,'s':sigma['s']**r}
        randomized_message={'M1':M['M1']**r,'M2':M['M2']}
        return (randomized_message,randomized_signature)
'''
groupObj = PairingGroup('BN254')
SPTS = SPTS(groupObj)
def start_bench(group):
    group.InitBenchmark()
    group.StartBenchmark(["RealTime"])

def end_bench(group):
    group.EndBenchmark()
    benchmarks = group.GetGeneralBenchmarks()
    real_time = benchmarks['RealTime']
    return real_time

def main(k,n):
    result=[n,k]   
    #setup
    setup_time=0
    for i in range(10):
        start_bench(groupObj)
        (mpk) = SPTS.PGen()
        setup_time += end_bench(groupObj)
    result.append(setup_time*100)

    # key gen
    keygen_time=0
    for i in range(10):
        start_bench(groupObj)
        (sk,vk,pk) = SPTS.kgen(mpk,k,n)
        keygen_time += end_bench(groupObj)
    result.append(keygen_time*100)
    key_size = sum([len(x) for x in serializeDict(sk, groupObj).values()])
    result.append(key_size)
 
 
    # Signing
    m = groupObj.random(ZR)
    id=mpk['g']**m
    M=SPTS.iDH(mpk,id,m)

    signing_time1=0
    for i in range(10):
        start_bench(groupObj)
        sigma = SPTS.par_sign(mpk,sk,M,k)
        signing_time1 += end_bench(groupObj)
    result.append(signing_time1*100)
    sig_size1 = sum([len(x) for x in serializeDict(sigma, groupObj).values()])
    result.append(sig_size1)

    #Par_Verify
    par_verify=0
    for i in range(10):
        start_bench(groupObj)
        (out) = SPTS.Par_verify(mpk,vk,pk,sigma,M,k)
        par_verify += end_bench(groupObj)
    result.append(par_verify*100)


    # Reconstruction
    recon_time=0
    for i in range(10):
        start_bench(groupObj)
        sigmaR = SPTS.reconst(sigma, k)
        recon_time += end_bench(groupObj)
    result.append(recon_time*100)

    # Verification
    verification_time=0
    for i in range(10):
        start_bench(groupObj)
        (out) = SPTS.verify(mpk,pk,M,sigmaR)
        verification_time += end_bench(groupObj)
    result.append(verification_time*100)

    rerandomization_time=0
    for i in range(10):
        start_bench(groupObj)
        (randomized_message,randomized_signature) = SPTS.Randomize(mpk,M,sigmaR)
        rerandomization_time += end_bench(groupObj)
    result.append(rerandomization_time*100)
    
    (out) = SPTS.verify(mpk,pk,randomized_message,randomized_signature)

    return result

book = Workbook()
data = book.active
title = ["n","k","setup_time", "keygen_time", "key_size" , "Signing_time1","Par_verify" ,"signature_size", "recons_time", "verification_time","rerandomization_time"]
data.append(title)

for n in range(3,19):
    data.append(main(n-2,n))
book.save("Result.xlsx")
'''