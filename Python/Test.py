...
#S.Decryption(mpk, ct1, M1, ct2, M2)
...
def run_round_trip(n,k,M,C):
    ...
    Ledger=dict.fromkeys(list(range(M+1)), [])
    Cus = []
    for i in range(C):
    ...
    for i in range(10):
        start_bench(groupObj)
        (Sgk_a,Vk_a,Pk_a) = S.AuKeygen(mpk,k,n)
    ...
    for i in range(10):
        start_bench(groupObj)
        (Vk_b,Sk_b) = S.MKeygen(mpk,len(Mer))
    ...
    for i in range(10):
        start_bench(groupObj)
        (Pk_b) = S.MRegister(mpk,Sgk_a,Vk_b,len(Mer),k)
    ...
    for i in range(10):
        start_bench(groupObj)
        (Sk_c,Pk_c,M_c) = S.CuKeyGen(mpk,len(Cus))
    ...
    for i in range(10):
        start_bench(groupObj)
        (cert_c) = S.CuRegister(mpk,Sgk_a,Pk_c,M_c,len(Cus),k)
    ...
    for i in range(10):
        start_bench(groupObj)
        (key, M_j, kprime,certprime,picol) = S.CuCreate(mpk,M_c[10],cert_c[10])
    ...
    for i in range(10):
        start_bench(groupObj)
        cert_j= S.AuCreate(mpk,Sgk_a,kprime,k,M_j,certprime,picol)
    ...
    for i in range(10):
        start_bench(groupObj)
        (pi,inp,R) = S.Spending(mpk, key, Pk_b[8], time,ID, Sk_c[10],cert_j,M_j)
    ...
    for i in range(10):
        start_bench(groupObj)
        out = S.Verification(mpk, Pk_a, M_j, pi, inp, R, Ledger, time,L1,L2, Pk_b[8],Sk_b,math.floor(len(Mer)/2)+1,Vk_b)
        Verification_time += end_bench(groupObj)
    Verification_time = Verification_time * 10
    result.append(Verification_time) 
    #(out2)= S.Verification(mpk,ct2,Rand2)
    ...
    for i in range(10):
        start_bench(groupObj)
        (out)= S.Decryption(mpk, ct1, 1, ct2, 2)
